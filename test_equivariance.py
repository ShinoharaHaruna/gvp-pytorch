...
class EquivarianceTest(unittest.TestCase):
    ...
    def test_gvp_sequence(self):
        model = (
            nn.Sequential(
                gvp.GVP(node_dim, node_dim), gvp.Dropout(0.1), gvp.LayerNorm(node_dim)
            )
            .to(device)
            .eval()
        )
    ...
    def test_gvp_conv_vector_gate(self):
        model = (
            gvp.GVPConv(node_dim, node_dim, edge_dim, vector_gate=True)
            .to(device)
            .eval()
        )
        model_fn = lambda h_V, h_E: model(h_V, edge_index, h_E)
    ...
    def test_gvp_conv_layer(self):
        ...
        model_fn = lambda h_V, h_E: model(h_V, edge_index, h_E, autoregressive_x=h_V)
        test_equivariance(model_fn, nodes, edges)
    ...
    def test_mqa_model(self):
        model = (
            gvp.models.MQAModel(node_dim, node_dim, edge_dim, edge_dim)
            .to(device)
            .eval()
        )
        model_fn = lambda h_V, h_E: (
            model(h_V, edge_index, h_E, batch=batch_idx),
    ...
    def test_cpd_model(self):
        model = (
            gvp.models.CPDModel(node_dim, node_dim, edge_dim, edge_dim)
            .to(device)
            .eval()
        )
        model_fn = lambda h_V, h_E: (
            model(h_V, edge_index, h_E, seq=seq),
            torch.zeros_like(nodes[1]),
        )
        test_equivariance(model_fn, nodes, edges)

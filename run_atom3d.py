...
parser.add_argument(
    "task",
    metavar="TASK",
    choices=["PSR", "RSR", "PPI", "RES", "MSP", "SMP", "LBA", "LEP"],
...
parser.add_argument(
    ...
    type=int,
    choices=[30, 60],
    help="identity cutoff for LBA, 30 (default) or 60",
    default=30,
)
parser.add_argument(
    "--batch", metavar="SIZE", type=int, default=8, help="batch size, default=8"
)
parser.add_argument(
    "--train-time",
    metavar="MINUTES",
...
parser.add_argument(
    "--epochs", metavar="N", type=int, default=50, help="training epochs, default=50"
)
parser.add_argument(
    "--test", metavar="PATH", default=None, help="evaluate a trained model"
)
parser.add_argument(
    "--lr", metavar="RATE", default=1e-4, type=float, help="learning rate"
)
parser.add_argument(
    "--load",
    metavar="PATH",
...
def load(model, path):
    ...
    for name, p in params.items():
        if (
            name in state_dict
            and name[:8] in ["layers.0", "layers.1"]
            and state_dict[name].shape == p.shape
        ):
            print("Loading", name)
...
def get_metrics(task):
    ...
    mean_correlations = {
        f"mean {k}": partial(v, glob=False) for k, v in correlations.items()
    }

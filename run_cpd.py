...
parser.add_argument(
    "--epochs", metavar="N", type=int, default=100, help="training epochs, default=100"
)
parser.add_argument(
    "--cath-data",
    metavar="PATH",
...
assert (
    sum(map(bool, [args.train, args.test_p, args.test_r])) == 1
...
dataloader = lambda x: torch_geometric.data.DataLoader(
    x,
    num_workers=args.num_workers,
    batch_sampler=gvp.data.BatchSampler(x.node_counts, max_nodes=args.max_nodes),
)
...
def main():
    ...
    cath = gvp.data.CATHDataset(
        path="data/chain_set.jsonl", splits_path="data/chain_set_splits.json"
    )

    trainset, valset, testset = map(
        gvp.data.ProteinGraphDataset, (cath.train, cath.val, cath.test)
    )
...
def test_recovery(model, dataset):
    ...
    for protein in tqdm.tqdm(dataset):
        ...
        sample = model.sample(h_V, protein.edge_index, h_E, n_samples=args.n_samples)
